...
st.subheader(" ")
st.subheader("A VAE can denoise a noisy image. In clicking on these buttons, you can admirate the VAE in action.")
st.subheader(" ")
...
st.subheader("A variational autoencoder also allows us to represent our data in a latent space of lower dimension than the base image. We can plot these clusters in order to see where our data lives")

image = Image.open("space.png")
st.image(image, caption = '2D dimension cluster', use_column_width=True)
st.header (" ")
st.subheader("You can now try to play with the coordinates of the latent space and see the image generated.")
st.header(" ")
col1, col2 = st.columns(2)
with col1 :
